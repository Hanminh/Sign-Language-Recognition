...
class SLR_Network(  nn.Module):
    def __init__(self, hidden_size= 1024, kernel_size=5,  num_classes= 1000, dictionary= None, T = 1., beam_size= 50, conv_type =9, conv_improve= False ):
    ...
    def forward(self, feat, vid_len):
        ...
        predicted_class = out_lstm["logits"].softmax(-1).argmax(-1)
    ...
    def get_loss(self, output, label):
        ...
        loss += total_loss['CrossEntropy']
        
        # total_loss['Dist'] = 10 * self.distillation_loss(
        ...
        #     output["conv_lstm_classifier"].view(-1, self.num_classes),
        #     label.view(-1)
        # )
        # loss += total_loss['Conv_CrossEntropy']
